...
class list_tasks(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['exp_sum'] = int(str(models.Profile.objects.filter(user=self.request.user)[0]))
        context['current_lvl'] = int(str(models.Profile.objects.filter(user=self.request.user)[0]))//100 + 1
        context['exp_left'] = 100 - int(str(models.Profile.objects.filter(user=self.request.user)[0]))%100
